...
class check_response():
    ...
    def check_result(response, params, expectNum = None):
        #由于搜索结果存在模糊匹配的情况，这里简单处理校验第一个返回结果的正确性
            ...
            if not response['subjects']:
                ...
                #再校验搜索条件q
                    ...
                    for word in params['q'].split(','):
                        title = [subject['title']]
                        casts = [i['name'] for i in subject['directors']]
...
class test_doubanSearch(object):
    ...
    def test_q(self):
        ...
        for q in qs:
            params = dict(q=q)
            f = partia;
            (test_doubanSearch.search, params)
    ...
    def test_param_q(self):
        #参数校验q,搜索类型：中文、英文、数字、特殊符号、None、空
    ...
    def test_param_tag(self):
        #参数校验tag， 搜索类型：中文、英文、数字、特殊符号、None、空
